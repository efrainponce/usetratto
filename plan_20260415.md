...
- [x] **16.1** Migration: agregar `permission_mode text DEFAULT 'public'` a `sub_item_columns`
- [x] **16.2** `lib/permissions.ts`: `userCanViewColumn(columnRef, userId, workspaceId)` — consulta `column_permissions`; si no hay registros → true (público); acepta `{type:'board'|'sub_item', id}`
- [x] **16.3** `lib/permissions.ts`: `resolveInheritedPermissions(subItemColId)` — si `permission_mode='inherit'`, resuelve via `boards.settings.sub_items_source_board_id` + `source_col_key`
- [x] **16.4** Snapshot engine: `POST /api/sub-items` + `/refresh` verifican `userCanViewColumn` sobre columna fuente antes de copiar; si false → omite silenciosamente (cached per col)
- [x] **16.5** `SubItemsView` NativeRow: columna con `user_access !== 'view'` → celda gris `bg-gray-50`, sin edit, sin tooltip; `ColumnCell` dispatcher documentado para futura reutilización
- [x] **16.6** `RelationCell` preview: TODO comment colocado — componente actual solo muestra label, el filtrado de fields del preview se implementará cuando se agregue el feature
- [x] **16.7** `ColumnSettingsPanel` tab Permisos en sub_item_columns: radio `permission_mode` con 3 opciones; detección automática vía `patchEndpoint.includes('/sub-item-columns/')`; lista de permisos se oculta en modo público/heredar

#### 16.B — Row-level constraints (restrict_to_own audit)
- [x] **16.8** `lib/permissions.ts`: `userCanAccessItem(itemId, userId, workspaceId, role?)` — admin bypass + owner + board_members (direct/team) + restrict_to_own
- [x] **16.9** `GET /api/sub-items`: `userCanAccessItem(itemId)` antes de devolver; 403 si false; columns vienen con `user_access` anotado
- [x] **16.10** `POST /api/sub-items/[id]/refresh` + `/expand` + `/import-children`: `userCanAccessItem` sobre item padre; 403 si false
- [x] **16.11** `GET /api/sub-item-views/[viewId]/data`: nativeHandler valida itemId con `userCanAccessItem`; board_items/board_sub_items handlers aplican filtro `owner_id = user` cuando membership tiene `restrict_to_own`
- [x] **16.12** `RelationCell` picker: `ProductPicker` usa `/api/items` que ya aplica `restrict_to_own` desde Fase 9 — no-op confirmado
- [x] **16.13** Audit de `createServiceClient()`: 4 rutas de sub-items sin `workspace_id` check identificadas y corregidas (refresh, expand, import-children + GET sub-items)
