      ...
      <div className="fixed right-0 top-0 h-full w-80 bg-white border-l border-gray-200 shadow-xl z-50 flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {tab === 'permisos' && permissionsEndpoint && (
            <div className="space-y-4">
              {/* Default access dropdown — for all columns */}
              <div className="space-y-3 pb-3 border-b border-gray-100">
                ...
                {savingDefaultAccess && (
                  <p className="text-[11px] text-gray-400">Guardando...</p>
                )}
              </div>

              {/* Permission Mode selector — only for sub-item columns */}
              {isSubItemColumn && (
                <div className="space-y-3 pb-3 border-b border-gray-100">
              ...
              {/* Permissions list — visibility depends on permission_mode (sub-item columns only) */}
              {permsLoading ? (
                ...
                <>
                  {isSubItemColumn && permissionMode === 'public' ? (
                    <p className="text-xs text-gray-500">
                      Esta columna es pública, los permisos no aplican.
                    </p>
                  ) : isSubItemColumn && permissionMode === 'inherit' ? (
                    <p className="text-xs text-gray-500">
                      Los permisos se heredan del origen.
                    </p>
                  ) : (
                    <>
