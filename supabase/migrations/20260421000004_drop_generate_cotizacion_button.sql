-- ═══════════════════════════════════════════════════════════════════════════════
-- DROP generate_cotización BUTTON COLUMN
-- ═══════════════════════════════════════════════════════════════════════════════
-- El CTA "Generar cotización" ahora vive en el footer del SubItemsView (dentro
-- del card de partidas). Quitar el button column auto-seeded de Oportunidades
-- + actualizar seed_system_boards para no inyectarlo en nuevos workspaces.
-- ═══════════════════════════════════════════════════════════════════════════════

-- 1. Drop existing button columns with action=generate_document en todos los workspaces
DELETE FROM board_columns
WHERE kind = 'button'
  AND settings->>'action' = 'generate_document';

-- 2. Reemplazar seed_system_boards() — misma definición que 20260421000003 pero sin
--    el INSERT del button column kind='button' con action='generate_document'.
...
BEGIN
  ...
  -- ── Default sub_item_views (opinionated graph, sin generar_cotizacion button) ─
