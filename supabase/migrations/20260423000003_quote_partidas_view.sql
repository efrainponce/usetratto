-- ═══════════════════════════════════════════════════════════════════════════════
-- Partidas view on Cotizaciones board
-- ═══════════════════════════════════════════════════════════════════════════════
-- Each quote (item in `quotes` board) owns an immutable snapshot of the products
-- that were in the originating opportunity's Catálogo at materialization time.
-- This view is the canvas where compras works (costs, validations) without ever
-- touching the opp's Catálogo.
--
-- 1. Backfill: for existing Cotizaciones boards, create the "Partidas" native
--    sub_item_view + its sub_item_columns (same shape as opp's Catálogo).
-- 2. Update seed_system_boards() so new workspaces get it automatically.
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── PART 1: Backfill existing Cotizaciones boards ──────────────────────────────
...
BEGIN
  ...
  LOOP
    -- Create the Partidas view if it doesn't exist
    SELECT id INTO v_view_id
    FROM sub_item_views
    WHERE board_id = v_quotes_id AND name = 'Partidas' AND type = 'native'
    LIMIT 1;

    IF v_view_id IS NULL THEN
      INSERT INTO sub_item_views (board_id, workspace_id, name, position, type, config)
      VALUES (v_quotes_id, v_workspace_id, 'Partidas', 0, 'native', '{}'::jsonb)
      RETURNING id INTO v_view_id;
    END IF;

    -- Seed columns (same shape as Catálogo, but no source_col_key — these are
    -- raw copies, not source-mapped)
    INSERT INTO sub_item_columns
      (board_id, view_id, col_key, name, kind, position, source_col_key, is_hidden, required, settings)
    VALUES
      (v_quotes_id, v_view_id, 'sku',          'SKU',             'text',    1, NULL, false, false, '{}'::jsonb),
      (v_quotes_id, v_view_id, 'descripcion',  'Descripción',     'text',    2, NULL, false, false, '{}'::jsonb),
      (v_quotes_id, v_view_id, 'foto',         'Foto',            'file',    3, NULL, false, false, '{"max_files":1}'::jsonb),
      (v_quotes_id, v_view_id, 'unit_price',   'Precio unitario', 'number',  4, NULL, false, false, '{"format":"currency"}'::jsonb),
      (v_quotes_id, v_view_id, 'cantidad',     'Cantidad',        'number',  5, NULL, false, false, '{"default_value":1}'::jsonb),
      (v_quotes_id, v_view_id, 'subtotal',     'Subtotal',        'formula', 6, NULL, false, false, '{"formula":"multiply","col_a":"cantidad","col_b":"unit_price","format":"currency"}'::jsonb)
    ON CONFLICT (board_id, col_key) DO NOTHING;
  END LOOP;
END $$;

-- ── PART 2: Extend seed_system_boards() for new workspaces ─────────────────────
...
BEGIN
  ...
  -- Cotizaciones: Partidas (native, immutable snapshot target)
  INSERT INTO sub_item_views (board_id, workspace_id, name, position, type, config) VALUES
  ...
  VALUES
    ...
    (v_quotes_id, v_partidas_view_id, 'descripcion',  'Descripción',     'text',    2, NULL, false, false, '{}'::jsonb),
    (v_quotes_id, v_partidas_view_id, 'foto',         'Foto',            'file',    3, NULL, false, false, '{"max_files":1}'::jsonb),
    (v_quotes_id, v_partidas_view_id, 'unit_price',   'Precio unitario', 'number',  4, NULL, false, false, '{"format":"currency"}'::jsonb),
    (v_quotes_id, v_partidas_view_id, 'cantidad',     'Cantidad',        'number',  5, NULL, false, false, '{"default_value":1}'::jsonb),
    (v_quotes_id, v_partidas_view_id, 'subtotal',     'Subtotal',        'formula', 6, NULL, false, false, '{"formula":"multiply","col_a":"cantidad","col_b":"unit_price","format":"currency"}'::jsonb);
