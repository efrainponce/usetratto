-- ═══════════════════════════════════════════════════════════════════════════════
-- Add `costo_unitario` to Catálogo board
-- ═══════════════════════════════════════════════════════════════════════════════
-- The catalog tracks what a product COSTS (supplier-facing). The selling price
-- is per-deal and lives on sub_items (unit_price), not here.
--
-- This column will be sensitive — compras/admin see it; vendedor does not.
-- The column_permissions layer handles that gating; not wired up in this
-- migration (admins see it by default; future migration adds role gating).
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── Backfill: add column to existing Catálogo boards ─────────────────────────

INSERT INTO board_columns (board_id, col_key, name, kind, position, is_system, settings)
SELECT b.id, 'costo_unitario', 'Costo unitario', 'number', 8, false,
       '{"format":"currency"}'::jsonb
FROM boards b
WHERE b.system_key = 'catalog'
  AND NOT EXISTS (
    SELECT 1 FROM board_columns bc WHERE bc.board_id = b.id AND bc.col_key = 'costo_unitario'
  );

-- ── Extend seed_system_boards() so new workspaces get it ─────────────────────
-- (single-focused patch: we inject the column right after Catálogo columns are
-- created, using dynamic SQL to avoid re-declaring the whole function.)

-- Minimal approach: run after seed completes for new workspaces via trigger or
-- just update the function body. Inline-update is cleanest.
...
BEGIN
  ...
  -- Catálogo (con costo_unitario opinionated)
  INSERT INTO board_columns (board_id, col_key, name, kind, position, is_system, settings) VALUES
    ...
    (v_catalog_id, 'unit_price',     'Precio venta',   'number',   5, false, '{"format":"currency"}'::jsonb),
    ...
    (v_catalog_id, 'costo_unitario', 'Costo unitario', 'number',   8, false, '{"format":"currency"}'::jsonb);
