...
**~sesión 3 — Mandar a cotización (snapshot RPC) + board Documentos + conditional_select + Expandir variantes**
- Snapshot pattern cotización: opp Catálogo tab → botón **Mandar a cotización** (reemplaza editor modal + botón PDF) → RPC `materialize_quote_from_opportunity` crea item en board `quotes`, copia sub_items L1+L2 con mapping por col_key, setea relations oportunidad/contacto/institucion (2-hop via contact), calcula monto. Migrations 003→008: vista Partidas en Cotizaciones + view-level cleanup (legacy "Sub-items" borrada, `src_*` col_keys renombrados a limpios, positions normalizadas)
- Board `documentos` nuevo (system_key='documents', folio DOC): umbrella para factura / oc_cliente / oc_proveedor / recepcion / devolucion. Cols name/stage/doc_type/direction/oportunidad/contacto/institucion/proveedor/monto/pdf_url/fecha_entrega/generated_by + Partidas sub_item_view (mismas cols que cotizaciones). `board_stages.settings.applies_to_doc_types` jsonb gatea stages por tipo. Reflejos en Opp/Contactos/Instituciones/Proveedores. Cotizaciones queda como board aparte (naming decision del user: "quotes super claro, documentos para el combo")
- Opinionated Catálogo: `proveedor` (relation→vendors) + `unidad` (select pza/caja/kg/m/m²/par/l) + `costo_unitario` (precio de venta es variable → vive en sub_items unit_price) + `tallas` (text CSV) + `colores_disponibles` (text CSV). Precio venta renombrado semánticamente
- Kind nuevo `conditional_select`: dropdown cuyas opciones vienen del item origen via `source_item_id` + settings.source_col_key. Migration 011 relaja constraint en board_columns + sub_item_columns. API `/api/sub-item-views/[viewId]/data` resuelve `conditional_options` server-side (batch-fetch board_columns + item_values, parsea CSV). SubItemsView renderiza case nuevo con SelectCell + opts dinámicas por row ("Sin opciones" si el catálogo no tiene listas). Seed automático en vista Variantes del Catálogo (talla/color) + vista Catálogo del Opp (talla/color a L2) + Partidas de Documentos
- RPC `expand_catalog_variants(catalog_item_id, use_tallas, use_colores)` + API `/api/items/[id]/expand-variants` + botón "Expandir variantes" en footer de SubItemsView (gated `boardSystemKey='catalog'` + native sin source) con modal checkbox tallas/colores → producto cartesiano idempotente (skip si combo ya existe, nombre "XS — Negro" o solo "XS" según lo marcado)
- 10 migrations 003→012 aplicadas en Supabase remoto. Typecheck limpio. Pendiente: RPC `materialize_document_from_opportunity(opp_id, doc_type, proveedor_id)` con filtro por proveedor del catalog item + botón "Generar OCs por proveedor" + keyboard nav en SubItemsView + UI de dropdown de etapa filtrada por doc_type en Documentos
